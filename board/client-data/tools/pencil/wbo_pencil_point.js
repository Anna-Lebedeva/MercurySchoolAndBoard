(function (global) {
    ...
    function pencilExtrapolatePoints(pts, x, y) {
        ...
        let ANGULARITY = 3; //The lower this number, the smoother the line
        let prev_values = pts[pts.length - 1].values; // Previous point
        let ante_values = pts[pts.length - 2].values; // Point before the previous one
        let prev_x = prev_values[prev_values.length - 2];
        let prev_y = prev_values[prev_values.length - 1];
        let ante_x = ante_values[ante_values.length - 2];
        let ante_y = ante_values[ante_values.length - 1];
        ...
        let vectx = x - ante_x,
            vecty = y - ante_y;
        let norm = Math.hypot(vectx, vecty);
        let dist1 = dist(ante_x, ante_y, prev_x, prev_y) / norm,
