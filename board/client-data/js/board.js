...
(function () {
	...
	window.addEventListener("wheel", event => window.scrollBy(0, event.deltaY));
})();

function resizeCanvas(m) {
	//Enlarge the canvas whenever something is drawn near its border
	var x = m.x | 0, y = m.y | 0
	var MAX_BOARD_SIZE_X = $('body').css('width').replace('px', ''); // Maximum value for x on the board
	var MAX_BOARD_SIZE_Y = Tools.server_config.MAX_BOARD_SIZE_Y || 65536; // Maximum value for y on the board
...
Tools.setScale = function setScale(scale) {
	var fullScale = Math.max(window.innerWidth, window.innerHeight) / Tools.server_config.MAX_BOARD_SIZE_Y;
