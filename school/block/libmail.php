...
class Mail
{
    ...
    {$filename = $_SERVER['DOCUMENT_ROOT'].$filename;
      $filename2 =iconv ('utf-8', 'windows-1251', $filename);
        ...
        {
            echo 'Файл '.$filename2.' не найден :(<br>';
            return FALSE;
        }
...
// если тип файла не указан, пытаемся определить mime по расширению
        ...
        // читаем файл
        $body.=chunk_split(base64_encode(file_get_contents($filename2)));
...
// если установлено подтверждение о доставке, берем адрес куда слать уведомление из адрес для ответа, либо из From
     ...
     * @param int $timeout
     */
    public function smtp_on($smtp_serv, $login, $code, $port = 25, $timeout = 5)
    {
    ...
    public function Send()
    {
        ...
        else // если через smtp
        {
            ...
            if (!$this->smtp['serv'] OR !$this->smtp['login'] OR !$this->smtp['code'] OR !$this->smtp['port'])
            {
                $this->status_mail['status'] = false;
...
// разбиваем (FROM - от кого) на юзера и домен. юзер понадобится в приветсвии с сервом
            ...
            fputs($smtp_conn, base64_encode($this->smtp['code'])."\r\n");
